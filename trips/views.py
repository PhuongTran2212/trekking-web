...
from .models import ChuyenDi, ChuyenDiThanhVien, ChuyenDiTimeline, ChuyenDiMedia
from .forms import TripFilterForm, ChuyenDiForm, TimelineFormSet
from .forms import SelectTrekFilterForm 
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                try:
                    ...
                    messages.error(request, "Lỗi hệ thống: Trạng thái mặc định không tồn tại.")
                    return render(request, 'trips/trip_form.html', {'form': form, 'cung_duong': cung_duong, 'page_title': 'Lên kế hoạch Chuyến đi'})
                ...
                messages.success(request, f"Đã tạo chuyến đi '{chuyen_di.ten_chuyen_di}'. Giờ hãy hoàn thiện kế hoạch của bạn!")
                return redirect('trips:update_trip', pk=chuyen_di.pk, slug=chuyen_di.slug)
    else:
        form = ChuyenDiForm()

    context = {'form': form, 'cung_duong': cung_duong, 'page_title': 'Lên kế hoạch Chuyến đi'}
...
# ==========================================================
# === 4. CÁC API CHO HÀNH ĐỘNG AJAX (Đầy đủ) ===
# ==========================================================
@login_required
@require_POST
def join_trip_request(request, pk):
    ...
    # TODO: Tạo thông báo cho trưởng đoàn
...
def approve_member(request, pk, member_id):
    ...
    membership.save()
    # TODO: Tạo thông báo cho người dùng được duyệt
...
def reject_member(request, pk, member_id):
    ...
    membership.save()
    # TODO: Tạo thông báo cho người dùng bị từ chối
...
def delete_trip_media(request, media_id):
    ...
    # Cho phép trưởng đoàn hoặc chính người đăng ảnh xóa
...
class SelectTrekForTripView(LoginRequiredMixin, ListView):
    ...
    paginate_by = 9 # Giảm xuống 9 để vừa 3x3

    def get_queryset(self):
        # THÊM ANNOTATE ĐỂ ĐẾM SỐ CHUYẾN ĐI
